            ...
            SampleSchema.model_validate(
                {
                    "id": sample.id,
                    "index_letter_id": (
                        sample.index_letter.id if sample.index_letter else None
                    ),
                    "strain_id": (sample.strain.id if sample.strain else None),
                    "storage_id": (
                        sample.storage.id if sample.storage else None
                    ),
                    "original_sample_number": sample.original_sample_number,
                    "source_id": (sample.source.id if sample.source else None),
                    "location_id": (
                        sample.location.id if sample.location else None
                    ),
                    "appendix_note_id": (
                        sample.appendix_note.id
                        if sample.appendix_note
                        else None
                    ),
                    "comment_id": (
                        sample.comment.id if sample.comment else None
                    ),

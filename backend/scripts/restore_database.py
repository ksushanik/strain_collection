...
class DatabaseRestore:
    ...
    def load_backup_metadata(
        ...
        if metadata_path.exists():
            try:
                ...
                self.logger.warning(
                    f"Ошибка чтения метаданных {metadata_path}: {e}"
                )
    ...
    def validate_backup_file(self, backup_file: str) -> bool:
        ...
        try:
            ...
            if "PostgreSQL" not in first_line and "pg_dump" not in first_line:
                ...
                if not any(
                    "PostgreSQL" in line or "pg_dump" in line or "SET" in line
            ...
            self.logger.info(
                f"Backup файл прошел валидацию: {backup_path.name}"
            )
            return True
    ...
    def create_pre_restore_backup(self) -> Optional[Path]:
        ...
        if backup_path:
            ...
            new_name = backup_path.name.replace(
                "strain_collection_full_",
    ...
    def restore_from_backup(
        ...
        try:
            ...
            self.logger.info(
                f"Текущая БД: {current_db_info.get('tables_count', 0)} таблиц, "
            ...
            if create_backup and current_db_info.get("tables_count", 0) > 0:
                ...
                if not pre_restore_backup:
                    self.logger.warning(
                        "Не удалось создать pre-restore backup, продолжаем..."
                    )
            ...
            self.logger.info(
                f"Команда: {' '.join(cmd[:-1])} --file=[SQL_FILE]"
            )

            result = subprocess.run(
                cmd, stderr=subprocess.PIPE, env=env, text=True
            )
            ...
            if restored_db_info.get("accessible"):
                self.logger.info(
                    f"Восстановление завершено. Таблиц: {restored_db_info.get('tables_count', 0)}, "
                ...
                self.logger.error(
                    "База данных недоступна после восстановления"
                )
                return False
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if tables:
                    ...
                    cursor.execute(
                        f"DROP TABLE IF EXISTS {tables_list} CASCADE"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Восстановление базы данных strain_collection из backup"
    )
    ...
    parser.add_argument(
        "--force", action="store_true", help="Не запрашивать подтверждение"
    )
    parser.add_argument(
        "--info-only",
        action="store_true",
    ...
    if metadata:
        ...
        print(
            f"   Размер: {metadata.get('file_size', 0) / (1024*1024):.1f} MB"
        )
    ...
    if current_db_info.get("accessible"):
        ...
        print(
            f"   ❌ Недоступна: {current_db_info.get('error', 'неизвестная ошибка')}"
        )

    # Предупреждения и подтверждения
    print("\n⚠️  ВНИМАНИЕ:")
    print("   - Восстановление заменит текущие данные")
    if not args.no_backup:
        print("   - Будет создан backup текущей БД")
    if args.drop_existing:
        print("   - Существующие таблицы будут удалены")

...
class DatabaseBackup:
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                stats = {}
                for table in ['collection_manager_strain', 'collection_manager_sample', 'collection_manager_storage']:
                    try:
    ...
    def create_backup(self, backup_type: str = 'full', compress: bool = True) -> Optional[Path]:
        ...
        try:
            ...
            # Выполнение backup'а
            self.logger.info(f"Выполнение команды: {' '.join(cmd[:-1])} [DB_NAME]")
            ...
            if backup_file.suffix in ['.sql', '.gz']:
                ...
                if metadata_file.exists():
                    ...
                    backups.append({
                        ...
                        'backup_type': 'unknown',
                        'creation_time': datetime.datetime.fromtimestamp(stat.st_mtime).isoformat(),
    ...
    def cleanup_old_backups(self, keep_days: int = 30, keep_count: int = 10):
        ...
        for i, backup in enumerate(backups):
            ...
            backup_date = datetime.datetime.fromisoformat(backup['creation_time'].replace('Z', '+00:00').replace('+00:00', ''))
            ...
            if backup_date < cutoff_date:
                ...
                try:
                    ...
                    self.logger.info(f"Удален старый backup: {backup['backup_file']}")
                    removed_count += 1
                    
                except Exception as e:
                    self.logger.error(f"Ошибка удаления backup {backup['backup_file']}: {e}")
            ...
            if backup_path.suffix == '.gz':
                ...
                with gzip.open(backup_path, 'rt') as f:
                    ...
                    for i, line in enumerate(f):
                        ...
                        if not (line.startswith('--') or line.startswith('SET') or 
                               line.startswith('CREATE') or line.startswith('INSERT')):
                            self.logger.warning(f"Подозрительная строка в backup: {line[:50]}...")
            else:
                # Проверка обычного SQL файла
                with open(backup_path, 'r') as f:
                    first_lines = [f.readline() for _ in range(10)]
                    if not any('PostgreSQL' in line or 'pg_dump' in line for line in first_lines):
                        self.logger.warning("Backup не содержит стандартных заголовков PostgreSQL")
...
def main():
    parser = argparse.ArgumentParser(description='Система backup базы данных strain_collection')
    parser.add_argument('action', choices=['create', 'list', 'cleanup', 'validate'],
                       help='Действие для выполнения')
    parser.add_argument('--type', choices=['full', 'schema'], default='full',
    ...
    parser.add_argument('--file', help='Имя файла для валидации (только для validate)')
    ...
    if args.action == 'create':
        ...
        print(f"✅ Очистка завершена (хранение: {args.keep_days} дней, минимум: {args.keep_count} файлов)")

...
from .models import (AppendixNote, Comment, IndexLetter, Location, Sample,
                     Source, Storage, Strain)
from .schemas import SampleSchema
from .utils import generate_batch_id, log_change, model_to_dict
    ...
    if "created_after" in request.GET:
        try:
            ...
            date_after = datetime.fromisoformat(
                request.GET["created_after"].replace("Z", "+00:00")
            )
    ...
    if "created_before" in request.GET:
        try:
            ...
            date_before = datetime.fromisoformat(
                request.GET["created_before"].replace("Z", "+00:00")
            )
            where_conditions.append("st.created_at <= %s")
        ...
        data.append(
            {
                ...
                if strain["created_at"]
                else None,
                "updated_at": strain["updated_at"].isoformat()
                if strain["updated_at"]
                else None,
            }
        )
    ...
    return Response(
        {
            ...
            "filters_applied": {
                ...
                "date_range": "created_after" in request.GET
                or "created_before" in request.GET,
            },
        }
    )
...
def get_strain(request, strain_id):
    ...
    try:
        ...
        samples_with_biochemistry = strain.samples.filter(
        ...
        return Response(
            {
                ...
                "samples_stats": {
                    ...
                    "photo_percentage": round(
                        (samples_with_photo / samples_count * 100)
                        if samples_count > 0
                        else 0,
                        1,
                    ),
                    "identified_percentage": round(
                        (samples_identified / samples_count * 100)
                        if samples_count > 0
                        else 0,
                        1,
                    ),
        ...
        return Response(
            {"error": "Штамм не найден"}, status=status.HTTP_404_NOT_FOUND
        )
...
def update_strain(request, strain_id):
    ...
    try:
        ...
        if (
            Strain.objects.filter(short_code=validated_data.short_code)
            .exclude(id=strain_id)
            .exists()
        ):
            return Response(
                {
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Updated strain: {strain.short_code} (ID: {strain.id})"
            )
        ...
        return Response(
            {"error": "Ошибки валидации данных", "details": e.errors()},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def delete_strain(request, strain_id):
    ...
    try:
        ...
        with transaction.atomic():
            strain.delete()
            logger.info(
                f"Deleted strain: {strain_info['short_code']} (ID: {strain_info['id']})"
            )
...
def list_samples(request):
    ...
    limit = min(
        int(request.GET.get("limit", 100)), 1000
    ...
    if "has_antibiotic_activity" in request.GET:
        ...
        sql_params.append(
            request.GET["has_antibiotic_activity"].lower() == "true"
        )
    ...
    return Response(
        {
            ...
            "filters_applied": {
                ...
                "has_antibiotic_activity": "has_antibiotic_activity"
                in request.GET,
...
def list_storage(request):
    ...
    for storage in Storage.objects.all():
        ...
        cell_info = {
            ...
            "strain_code": actual_sample.strain.short_code
            if actual_sample and actual_sample.strain
            else None,
    ...
    return Response(
        {
            ...
            "samples_analysis": {
                ...
                "identified": Sample.objects.filter(
                    is_identified=True
                ).count(),
                "with_antibiotic_activity": Sample.objects.filter(
                    has_antibiotic_activity=True
                ).count(),
                "with_genome": Sample.objects.filter(has_genome=True).count(),
                "with_biochemistry": Sample.objects.filter(
                    has_biochemistry=True
                ).count(),
    ...
    try:
        ...
        class CreateSampleSchema(BaseModel):
            ...
            comment_id: Optional[int] = Field(
                None, ge=1, description="ID комментария"
            )
            ...
            has_genome: bool = Field(
                default=False, description="Есть ли геном"
            )
        ...
        if validated_data.storage_id:
            if not Storage.objects.filter(
                id=validated_data.storage_id
            ).exists():
                return Response(
                    {
            ...
            existing_sample = (
                Sample.objects.filter(storage_id=validated_data.storage_id)
                .exclude(comment_id=2)
                .first()
            )  # Исключаем помеченные как свободные
        ...
        if validated_data.index_letter_id:
            if not IndexLetter.objects.filter(
                id=validated_data.index_letter_id
            ).exists():
                return Response(
                    {
        ...
        if validated_data.appendix_note_id:
            if not AppendixNote.objects.filter(
                id=validated_data.appendix_note_id
            ).exists():
                return Response(
                    {
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Created sample {sample.id} for strain {validated_data.strain_id}"
            )
        ...
        return Response(
            {"error": "Ошибка валидации данных", "details": e.errors()},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        return Response(
            {
                ...
                "appendix_note": {
                    "id": sample.appendix_note.id
                    if sample.appendix_note
                    else None,
                    "text": sample.appendix_note.text
                    if sample.appendix_note
                    else None,
                }
                if sample.appendix_note
    ...
    try:
        ...
        class UpdateSampleSchema(BaseModel):
            ...
            is_identified: Optional[bool] = Field(
                None, description="Идентифицирован ли"
            )
            has_antibiotic_activity: Optional[bool] = Field(
                None, description="Есть ли антибиотическая активность"
            )
            has_genome: Optional[bool] = Field(
                None, description="Есть ли геном"
            )
            has_biochemistry: Optional[bool] = Field(
                None, description="Есть ли биохимия"
            )
            seq_status: Optional[bool] = Field(
                None, description="Статус секвенирования"
            )
        ...
        if validated_data.storage_id is not None:
            ...
            existing_sample = (
                ...
                .exclude(comment_id=2)
                .first()
            )  # Исключаем текущий образец и помеченные как свободные
        ...
        if (
            validated_data.source_id is not None
            and not Source.objects.filter(id=validated_data.source_id).exists()
        ):
            return Response(
                {
                    "error": f"Источник с ID {validated_data.source_id} не найден"
        ...
        if (
            validated_data.location_id is not None
            and not Location.objects.filter(
                id=validated_data.location_id
            ).exists()
        ):
            return Response(
        ...
        if (
            validated_data.index_letter_id is not None
            and not IndexLetter.objects.filter(
                id=validated_data.index_letter_id
            ).exists()
        ):
            return Response(
        ...
        if (
            validated_data.appendix_note_id is not None
            and not AppendixNote.objects.filter(
                id=validated_data.appendix_note_id
            ).exists()
        ):
            return Response(
        ...
        if (
            validated_data.comment_id is not None
        ...
        with transaction.atomic():
            ...
            if update_fields:
                ...
                logger.info(
                    f"Updated sample {sample.id}, fields: {update_fields}"
                )
        ...
        return Response(
            {
                ...
                "index_letters": [
                    {"id": letter.id, "letter_value": letter.letter_value}
...
def bulk_delete_samples(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Bulk deleted {deleted_count} samples: {sample_ids} (batch: {batch_id})"
            )
...
def bulk_update_samples(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Bulk updated {updated_count} samples with data: {filtered_update_data} (batch: {batch_id})"
            )
...
def bulk_update_strains(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Bulk updated {updated_count} strains with data: {filtered_update_data} (batch: {batch_id})"
            )
...
def bulk_export_samples(request):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            fields = (
                request.data.get("fields", "").split(",")
                if request.data.get("fields")
                else []
            )
            _include_related = request.data.get("include_related", "true")  # noqa: F841
        else:
            sample_ids = request.GET.get("sample_ids", "")
            format_type = request.GET.get("format", "csv").lower()
            fields = (
                request.GET.get("fields", "").split(",")
                if request.GET.get("fields")
                else []
            )
            _include_related = request.GET.get("include_related", "true")  # noqa: F841
        ...
        if _include_related.lower() == "true":
            samples = samples.select_related(
                "strain", "storage", "source", "location"
            )
        ...
        if not fields or not any(f.strip() for f in fields):
            ...
            export_fields = [
                f.strip() for f in fields if f.strip() in available_fields
            ]
        ...
        for s in samples:
            ...
            for f in export_fields:
                if f == "id":
                    ...
                    row[available_fields[f]] = (
                        s.strain.short_code if s.strain else ""
                    )
                elif f == "strain_identifier":
                    row[available_fields[f]] = (
                        s.strain.identifier if s.strain else ""
                    )
                    ...
                    row[available_fields[f]] = (
                        "Да" if s.is_identified else "Нет"
                    )
                elif f == "has_antibiotic_activity":
                    row[available_fields[f]] = (
                        "Да" if s.has_antibiotic_activity else "Нет"
                    )
                elif f == "has_genome":
                    row[available_fields[f]] = "Да" if s.has_genome else "Нет"
                elif f == "has_biochemistry":
                    row[available_fields[f]] = (
                        "Да" if s.has_biochemistry else "Нет"
                    )
                elif f == "seq_status":
                    row[available_fields[f]] = "Да" if s.seq_status else "Нет"
                elif f == "source_organism":
                    row[available_fields[f]] = (
                        s.source.organism_name if s.source else ""
                    )
                elif f == "source_type":
                    row[available_fields[f]] = (
                        s.source.source_type if s.source else ""
                    )
                    ...
                    row[available_fields[f]] = (
                        f"{s.storage.box_id}:{s.storage.cell_id}"
                        if s.storage
                        else ""
                    )
                elif f == "created_at":
        ...
        if format_type == "json":
            ...
            try:
                ...
                response[
                    "Content-Disposition"
                ] = 'attachment; filename="samples_export.xlsx"'
                return response
            except ImportError:
                return Response(
                    {"error": "Excel экспорт недоступен"}, status=500
                )
        else:
            # CSV
            output = StringIO()
            ...
            response[
                "Content-Disposition"
            ] = 'attachment; filename="samples_export.csv"'
            return response
...
def bulk_export_strains(request):
    ...
    try:
        ...
        for strain in strains:
            ...
            for field in export_fields:
                if field == "id":
                    ...
                    row[available_fields[field]] = (
                        strain.rcam_collection_id or ""
                    )
                elif field == "created_at":
                    row[available_fields[field]] = (
                        strain.created_at.strftime("%Y-%m-%d %H:%M:%S")
                        if strain.created_at
                        else ""
                    )
                elif field == "updated_at":
                    row[available_fields[field]] = (
                        strain.updated_at.strftime("%Y-%m-%d %H:%M:%S")
                        if strain.updated_at
                        else ""
                    )
            data.append(row)
        ...
        if format_type == "json":
            ...
            response[
                "Content-Disposition"
            ] = 'attachment; filename="strains_export.json"'
            return response
            ...
            try:
                ...
                response[
                    "Content-Disposition"
                ] = 'attachment; filename="strains_export.xlsx"'
                return response
            ...
            response[
                "Content-Disposition"
            ] = 'attachment; filename="strains_export.csv"'
            return response
...
def list_storage_summary(request):
    ...
    with connection.cursor() as cursor:
        ...
        for row in cursor.fetchall():
            ...
            boxes.append(
                {"box_id": box_id, "occupied": occupied, "total": total}
            )
            total_boxes += 1
            total_cells += total
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for i in range(11, -1, -1):
                date = datetime(now.year, now.month, 1) - timedelta(
                    days=32 * i
                )

...
from .models import (
    ...
    Storage, Strain, Sample, ChangeLog
)
    ...
    def is_occupied(self, obj):
        try:
            ...
            if sample.strain:
                ...
                return format_html('<span style="color: orange;">Свободная ячейка</span>')
        except:
...
class SampleInline(admin.TabularInline):
    model = Sample
    extra = 0
    fields = ['storage', 'original_sample_number', 'has_photo', 'is_identified']
...
class StrainAdmin(admin.ModelAdmin):
    list_display = [
        'short_code', 'rrna_taxonomy', 'rcam_collection_id', 
...
class SampleAdmin(admin.ModelAdmin):
    list_display = [
        'get_strain_code', 'original_sample_number', 'get_storage_info',
        'has_photo', 'is_identified', 'has_antibiotic_activity'
    ]
    list_filter = [
        'has_photo', 'is_identified', 'has_antibiotic_activity',
        'has_genome', 'has_biochemistry', 'seq_status',
        'strain__rrna_taxonomy', 'location'
    ]
    search_fields = [
        'strain__short_code', 'strain__identifier', 'original_sample_number',
        'storage__box_id', 'storage__cell_id'
    ]
...
class ChangeLogAdmin(admin.ModelAdmin):
    list_display = [
        'created_at', 'content_type', 'object_id', 'action', 
        'user_info', 'get_short_comment'
    ]
    list_filter = ['action', 'content_type', 'created_at']
    search_fields = ['object_id', 'user_info', 'comment']
    readonly_fields = [
        'content_type', 'object_id', 'action', 'old_values', 'new_values',
        'user_info', 'ip_address', 'user_agent', 'comment', 'batch_id', 'created_at'
    ]
    ordering = ['-created_at']

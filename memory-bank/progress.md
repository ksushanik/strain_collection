...
- **Фотогалерея образцов** 🔧 *Milestone 1.4*
  - ⏳ Backend: модель `SamplePhoto`, API загрузки/удаления, thumbnail
  - ⏳ Удалить поле `has_photo`; пересчитать статистики на основе `photos.exists()`
  - ⏳ Frontend: блок загрузки фото (Add/Edit Sample), галерея в SampleDetail
  - ⏳ Обновить фильтры, бейджи, аналитические метрики «Есть фото»

...
clear-prod-data:
	@echo "🧹 Очистка данных на продакшн сервере..."
	@./scripts/clear_production_data.sh

clear-prod-data-force:
	@echo "🧹 Принудительная очистка данных на продакшн сервере..."
	@./scripts/clear_production_data.sh --force

clear-prod-data-no-backup:
	@echo "🧹 Очистка данных на продакшн сервере БЕЗ backup..."
...
cleanup-disk-force:
	@echo "🧹 Принудительная очистка диска на продакшн сервере..."
	@./scripts/cleanup_disk_space.sh --force
...
help:
	...
	@echo "  make clear-prod-data — очистить данные на продакшн сервере (с подтверждением)";
	@echo "  make clear-prod-data-force — принудительно очистить данные на продакшн сервере";
	@echo "  make clear-prod-data-no-backup — очистить данные БЕЗ backup (нехватка места)";
	@echo "  make cleanup-disk — очистить диск на продакшн сервере (Docker, логи, кэш)";
	@echo "  make cleanup-disk-force — принудительно очистить диск на продакшн сервере";

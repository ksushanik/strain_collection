...
class BackupCronSetup:
    ...
    def __init__(self, project_path: str = None):
        self.project_path = Path(project_path or os.path.dirname(__file__)).parent
    ...
    def install_cron_entries(self, entries: list):
        ...
        filtered_lines = [line for line in lines 
                         if 'strain_collection' not in line and 'backup_database.py' not in line]
    ...
    def show_current_entries(self):
        ...
        for line in lines:
            if 'strain_collection' in line and line.startswith('#'):
                ...
                if line.strip() == '' or line.startswith('#') or 'backup_database.py' in line:
                    strain_entries.append(line)
                else:
                    in_strain_section = False
    ...
    def test_backup_command(self):
        ...
        try:
            ...
            cmd = [str(python_path), str(self.backup_script), 'create', '--type', 'schema']
...
def main():
    parser = argparse.ArgumentParser(description='Настройка автоматических backup\'ов через cron')
    parser.add_argument('action', choices=['install', 'remove', 'show', 'test'],
